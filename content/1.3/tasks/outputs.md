...
- One or more files (as indicated by the `+` postfix quantifier) with the `.csv` extension in the working directory that are collected into an array by the [`glob`](@/1.3/standard-library/file-functions/glob.md) function.

See the [WDL Value Serialization](@/1.3/appendices/appendix-a.md) section for more details.
...
Another common pattern is to use the [`glob`](@/1.3/standard-library/file-functions/glob.md) function to define outputs that might contain zero, one, or many files.
