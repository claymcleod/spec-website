...
<span class="wdl-badge wdl-badge-deprecated">Deprecated</span> `docker` is supported as an alias for `container` with the exact same semantics. Exactly one of the `container` or `docker` attributes is required. The `docker` alias is deprecated and will be removed in WDL 2.0.
