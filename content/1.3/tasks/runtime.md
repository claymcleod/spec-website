...
The `runtime` section is essentially the same as the [`requirements`](@/1.3/tasks/requirements.md) section, with the only difference being that arbitrary attributes *are* allowed in the `runtime` section. All attributes defined in the `requirements` section have the same semantics when used in the `runtime` section and are considered as reserved.
