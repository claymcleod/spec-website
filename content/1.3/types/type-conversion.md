...
The execution engine is also responsible for converting (or "serializing") input values when constructing commands, as well as "deserializing" command outputs. For more information, see the [Command Section](@/1.3/tasks/command.md) and the more extensive Appendix on [WDL Value Serialization and Deserialization](@/1.3/appendices/appendix-a.md).
...
The [`read_lines`](@/1.3/standard-library/file-functions/read_lines.md) function presents a special case in which the `Array[String]` value it returns may be immediately coerced into other `Array[P]` values, where `P` is a primitive type. See [Appendix A](@/1.3/appendices/appendix-a.md) for details and best practices.
...
This constraint propagates into compound types. For example, an `Array[T?]` can contain both optional and non-optional elements. This facilitates the common idiom [`select_first([expr, default])`](@/1.3/standard-library/array-functions/select_first.md), where `expr` is of type `T?` and `default` is of type `T`, for converting an optional type to a non-optional type. However, an `Array[T?]` could not be passed to the [`sep`](@/1.3/standard-library/array-string-functions/sep.md) function, which requires an `Array[T]`.

There are two exceptions where coercion from `T?` to `T` is allowed:

* [String concatenation in expression placeholders](@/1.3/expressions/string-interpolation.md#concatenation-of-optional-values)
* [Equality and inequality comparisons](@/1.3/expressions/operators.md#equality-and-inequality-comparison-of-optional-types)
