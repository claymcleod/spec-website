...
* A `String` represents a unicode character string following the format described [below](@/1.3/types/primitive-types/strings.md).
...
JSON has a single numeric type - it does not differentiate between integral and floating point values. A JSON `number` is always deserialized to a WDL `Float`, which may then be [coerced](@/1.3/types/type-conversion.md#type-coercion) to an `Int` if necessary.
