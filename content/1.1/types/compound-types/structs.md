...
`Struct` definitions are top-level WDL elements, meaning they exist at the same level as [import](@/1.1/wdl-documents/import-statements.md), [`task` definition](@/1.1/tasks/_index.md), and [`workflow` definition](@/1.1/workflows/_index.md) definitions. A struct cannot be defined within a task or workflow body.
...
task greet_person {
  ...
  if ~{defined(person.income)}; then
    if [ ~{round(select_first([person.income]).amount)} -gt 1000 ]; then
