...
description = "Deprecated placeholder options: sep, true/false, and default"
weight = 80
+++
...
* [`sep`](@/1.1/expressions/placeholder-options.md#sep): convert an array to a string using a delimiter; e.g., `~{sep=", " array_value}`.
...
The `sep` option can be replaced with a call to the <span class="wdl-badge wdl-badge-new">New in 1.1</span> [`sep`](@/1.1/standard-library/array-string-functions/sep.md) function:
...
The `default` option can be replaced in several ways - most commonly with an `if-then-else` expression or with a call to the [`select_first`](@/1.1/standard-library/array-functions/select_first.md) function.
