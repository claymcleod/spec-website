...
* A [WDL `task`](@/1.1/tasks/_index.md) is a namespace consisting of:
    * `input`, `output`, and private declarations
    * A [`runtime`](@/1.1/tasks/runtime.md) namespace that contains all the runtime attributes
* A [WDL `workflow`](@/1.1/workflows/_index.md) is a namespace consisting of:
    * `input`, `output`, and private declarations
    * The [`call`s](@/1.1/workflows/call.md) made to tasks and subworkflows within the body of the workflow.
...
However, two sub-namespaces imported into the same parent namespace are allowed to contain the same names. For example, two documents with different namespace identifiers `foo` and `bar` can both have a task named `baz`, because the [fully-qualified names](@/1.1/workflows/qualified-names.md) of the two tasks would be different: `foo.baz` and `bar.baz`.
