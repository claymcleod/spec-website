...
Task and workflow inputs may be passed in from an external source, or they may be specified in the WDL document itself using literal values. Input, output, and other declaration values may also be constructed at runtime using [expressions](@/1.2/expressions/_index.md) that consist of literals, identifiers (references to [declarations](@/1.2/expressions/declarations.md) or [call](@/1.2/workflows/call.md) outputs), built-in [operators](@/1.2/expressions/operator-precedence.md), and [standard library functions](@/1.2/standard-library/_index.md).
