...
Any WDL string expression may contain one or more "placeholders" of the form `~{*expression*}`, each of which contains a single expression. Placeholders of the form `${*expression*}` may also be used interchangably, but their use is discouraged for reasons discussed in the [command section](@/1.2/tasks/command.md) and may be deprecated in a future version of the specification.
...
Compound types cannot be implicitly converted to `String`s. To convert an `Array` to a `String`, use the `sep` function: `~{sep(",", str_array)}`. See the guide on WDL value serialization for more details and examples.
