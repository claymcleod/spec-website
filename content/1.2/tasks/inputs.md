...
- When multiple input declarations refer to the same canonicalized `File` or `Directory` (i.e., they [compare as equal](@/1.2/types/primitive-types/strings.md#path-canonicalization-and-validation)), the execution engine should localize the resource once, and all references to those declarations should resolve to the same localized path.
