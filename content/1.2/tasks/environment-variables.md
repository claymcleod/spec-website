...
within the task. This applies to declarations within the input section as well as [private declarations](@/1.2/tasks/private-declarations.md). An important thing to note is that while the `env` modifier makes the declaration available in the
...
When an input is annotated with `env` it is the engine's responsibility to serialize the value appropriately into a string (see section on [Serialization of WDL values](@/1.2/appendices/appendix-a.md))
