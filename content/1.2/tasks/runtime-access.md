...
This information is provided by the `task` variable, which is implicitly defined by the execution engine. The type of `task` is a [scoped type](@/1.2/types/hidden-types.md#task-hidden-scoped-type) with the following members:
...
* `ext`: An `Object` containing execution engine-specific attributes, or the empty `Object` if there aren't any. Members of `ext` should be considered optional. It is recommended to only access a member of `ext` using [string interpolation](@/1.2/types/primitive-types/strings.md) to avoid an error if it is not defined.
