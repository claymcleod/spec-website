...
WDL has a special value `None` whose meaning is "an undefined value". The `None` value has the (hidden) type [`Union`](@/1.2/types/hidden-types.md#union-hidden-type), meaning `None` can be assigned to an optional declaration of any type.
...
For more details, see the sections on [Input Type Constraints](@/1.2/tasks/inputs.md#input-type-constraints) and [Optional Inputs with Defaults](@/1.2/tasks/inputs.md#optional-inputs-with-defaults).
