...
For more details see the section on [Input Type Constraints](@/1.2/tasks/inputs.md#input-type-constraints).
...
A `Map` is insertion-ordered, meaning the order in which elements are added to the `Map` is preserved, for example when [converting a `Map` to an array of `Pair`s](@/1.2/standard-library/map-functions/as_pairs.md).
