...
* [WDL document](@/1.2/introduction/_index.md)
    * The namespace of an [imported](@/1.2/introduction/_index.md) document equals that of the basename of the imported file by default, but may be aliased using the `as <identifier>` syntax.
...
* A [WDL `task`](@/1.2/tasks/_index.md) is a namespace consisting of:
    * `input`, `output`, and private declarations
    * A [`requirements`](@/1.2/tasks/requirements.md) namespace that contains all the runtime requirements
* A [WDL `workflow`](@/1.2/workflows/_index.md) is a namespace consisting of:
    * `input`, `output`, and private declarations
    * The [`call`s](@/1.2/workflows/call.md) made to tasks and subworkflows within the body of the workflow.
...
* A [`Struct` instance](@/1.2/types/compound-types/structs.md): is a namespace consisting of the members defined in the struct. This also applies to `Object` instances.
...
However, two sub-namespaces imported into the same parent namespace are allowed to contain the same names. For example, two documents with different namespace identifiers `foo` and `bar` can both have a task named `baz`, because the [fully-qualified names](@/1.2/workflows/qualified-names.md) of the two tasks would be different: `foo.baz` and `bar.baz`.
