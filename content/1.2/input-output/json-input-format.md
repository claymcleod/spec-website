...
[Requirement](@/1.2/tasks/requirements.md) and [hint](@/1.2/tasks/hints.md) attributes can be specified (or overridden) for any task in the JSON input file. To differentiate requirements and hints from task inputs, the `requirements` or `hints` namespace is added after the task name.
